...
class LocalTesseractOcrExecutor(OcrExecutor):
    def __init__(self, *, page_timeout_seconds: int | None = None) -> None:
        self.page_timeout_seconds = page_timeout_seconds or _local_ocr_page_timeout_seconds()
    ...
    def _ocr_images(self, sample: SampleFile, images: list[Image.Image]) -> tuple[OcrDocumentResult, list[OcrPageResult]]:
        ...
        for index, image in enumerate(images, start=1):
            ...
            try:
                pages.append(ocr_pil_image(sample, image, index, page_count, page_start, timeout_seconds=self.page_timeout_seconds))
            except Exception as error:  # noqa: BLE001
                pages.append(
                    OcrPageResult(
...
def _local_ocr_page_timeout_seconds() -> int | None:
    value = os.environ.get("SUNSHINE_LOCAL_OCR_PAGE_TIMEOUT_SECONDS", "45")
    try:
        parsed = int(value)
    except ValueError:
        return 45
