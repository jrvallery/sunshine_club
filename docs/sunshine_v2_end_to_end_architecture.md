...
- A local smoke benchmark on 2026-05-28 generated `.local/provider-benchmark-canonical-samples.json` from the QA sample root and ran `current,docling` over two small `image_scan,newspaper_packet` samples with a 10 MB cap. Both providers extracted successfully with OK quality, but both remained `needs_segmentation_review` because the segmentation-required sample did not expose page-level structure ready for boundary review. Current averaged about 2.84 seconds; Docling averaged about 24.57 seconds on the VM CPU path. This is not enough evidence to promote Docling as the default.
