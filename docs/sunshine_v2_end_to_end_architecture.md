...
- Postgres client and Compose service exist; V2 migrations now include run/results/model/provider/segment/review/golden-label/chunk/embedding tables, and the API exposes Postgres runtime, run detail, run listing, review-item, review-decision, golden-label, file search/facet, file detail/text/preview, and file-to-review enqueue surfaces. The Files dashboard can inspect and enqueue V2 Postgres results via a source selector. Dashboard runtime still needs to move remaining run actions and other legacy views from SQLite to Postgres as the authoritative store.
