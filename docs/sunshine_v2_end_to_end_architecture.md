...
- The Golden Labels dashboard has a source selector for SQLite versus V2 Postgres labels; Postgres labels can be inspected, exported, and used to build the local semantic index, while edit/delete/file-open controls remain limited to the legacy SQLite write path until V2 label edit endpoints are promoted.
