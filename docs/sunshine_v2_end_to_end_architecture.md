...
- `GET /admin/system/postgres-runtime/runs/{run_key}/segments` exposes document-segment rows directly from Postgres, including page ranges and review-required counts, so clients can inspect scrapbook/newspaper split proposals without loading the full run report.
- `GET /admin/system/postgres-runtime/runs/{run_key}/chunks` and `/chunk-embeddings` expose downstream chunk text snippets and embedding status directly from Postgres, so reviewers can trace whether a proposed scrapbook/newspaper page-range segment produced usable retrieval units before any physical split/export is considered.
...
- `apps/api/services/semantic_search.py` exposes local-only, citation-first semantic search over the Qdrant retrieval provider, and `POST /admin/search/semantic` returns matches with source path, relative path, run key, content class, primary tag, route status, review status, chunk ID, segment ID, segment type/title, page range, snippet, score, explanation, and provider attempt metadata.
- `POST /admin/search/semantic` has first-class filters for run key, primary tag, content class, route status, review status, segment type, and chunk kind, plus arbitrary Qdrant metadata filters. This lets the dashboard search scrapbook/newspaper segment proposals without forcing reviewers to inspect raw payload JSON.
...
- Cortex reranking uses the local SQLite model-call cache when configured through `SUNSHINE_MODEL_CACHE_PATH`; rerank cache hits remain visible in retrieval metadata without being counted as new model calls.
...
- `sample_pipeline.py` now delegates extraction, extraction validation/repair, quality gates, chunking, and embedding behavior to service/provider modules instead of owning duplicate implementations. The module remains a compatibility CLI while graph nodes and tests converge on the shared services.
...
- `PostgresPipelineStore` imports graph provider-selection rows into `pipeline_provider_selections`, preserving selected provider, preferred provider, configured provider, fallback chain, skipped providers, and selection reason so V2 reports can distinguish "what provider was chosen" from "what provider attempts executed."
- `PostgresPipelineStore` imports extraction validation, repair, quality-gate, and chunking-result rows into `pipeline_quality_checks`, preserving check type, status, quality, review requirement, chunk/embed eligibility, provider, strategy, warnings, and raw normalized row for durable extraction-quality review.
- `PostgresPipelineStore` imports retrieval results, semantic examples, LLM tag inspections, tag candidates, confidence calibrations, placement proposals, and route decisions into `pipeline_tagging_evidence`, preserving evidence type, provider/model, primary tag, confidence, assignment source, route status, placement status, warnings, evidence, and the raw normalized row.
- `PostgresPipelineStore` imports source identity, file probes, sample inputs, document structure, and import-result artifacts into `pipeline_file_metadata`, preserving file IDs, hashes, sizes, media/probe status, page/text counts, sample grouping, final class/strategy, and raw normalized rows for durable file-level audit.
- `PostgresPipelineStore` imports `artifact-manifest.json` entries into `pipeline_artifacts`, preserving artifact name, path, kind, existence, row count, size, hash, notes, and the raw manifest entry so run-owned artifact evidence is queryable from Postgres.
- `PostgresPipelineStore` imports extraction results, OCR document/page rows, and embedding result summaries into `pipeline_processing_artifacts`, preserving provider/model, status, quality, strategy, page numbers, text lengths, embedding counts, dimensions, cache hits/misses, warnings, and raw normalized rows.
...
- `GET /admin/system/postgres-runtime/runs/{run_key}/artifacts` exposes imported V2 artifact rows directly from Postgres, including raw-provider snapshot references, without requiring clients to load the full run report.
- `GET /admin/system/postgres-runtime/runs/{run_key}/model-usage` exposes the same model-usage summary shape as the legacy run API from imported Postgres rows, so V2 clients can inspect local model calls without loading the full run report.
...
- Postgres-backed run reports include provider selections in the `Providers` tab, including selected provider, preferred/configured provider, fallback chain, skipped providers, and selection reason.
- `GET /admin/system/postgres-runtime/runs/{run_key}/results` exposes normalized per-file results directly from Postgres with route and content-class counts, giving the dashboard a lightweight run file table before loading deeper review/provider/tagging details.
- `GET /admin/system/postgres-runtime/runs/{run_key}/provider-attempts` and `/provider-selections` expose provider attempt/selection rows directly from Postgres so the dashboard can lazy-load provider diagnostics without fetching the full report.
- Postgres-backed run reports include an `Extraction` tab for validation, repair, quality-gate, and chunking-result rows so operators can inspect why a file was allowed downstream or routed to review.
...
- Postgres-backed run reports include tagging evidence in the `Tags` tab, so deterministic rules, semantic matches, LLM tag inspection, confidence calibration, placement proposals, and route decisions can be inspected from one durable report surface.
- Postgres-backed run reports include file metadata in a `Metadata` tab, so source identity, probe results, sample manifest rows, structure rows, and import outcomes can be inspected without opening JSONL artifacts.
- `GET /admin/system/postgres-runtime/runs/{run_key}/quality-checks`, `/tagging-evidence`, `/file-metadata`, `/processing-artifacts`, and `/parser-results` expose the same detailed run-report tables as independent local endpoints. This supports cleaner dashboard tabs, incremental polling while a run is active, and targeted inspection of OCR/parser quality before scrapbook/newspaper segment decisions are reviewed.
- Postgres-backed run reports include imported artifact manifest rows in an `Artifacts` tab, so operators can inspect missing/generated artifacts, row counts, sizes, and hashes from the durable V2 report.
- Postgres-backed run reports now expose imported parser/OCR result rows through the existing `OCR` tab, including parser status, OCR document/page artifacts, quality, provider, and parser-review summary buckets.
- Postgres-backed run reports expose extraction-result rows alongside extraction quality checks and embedding-result/cache summaries alongside chunk and embedding rows.
...
- Raw-provider artifact reference rows include source path, source-relative path, sample path, artifact path, artifact-relative path, size, and hashes, so the reference row can be attached to a source file without opening the raw snapshot payload.
- `PostgresPipelineStore` imports raw-provider artifact references into `pipeline_artifacts` alongside manifest entries, so V2 run reports can retain durable path/hash evidence for Docling/local parser snapshots after filesystem artifact import.
- Non-current local parser providers now also emit `parser_extraction` model-usage rows from graph extraction attempts, so Docling/MinerU/RAGFlow/Unstructured runs are counted in usage reports even when they do not emit page-level OCR model rows.
...
- Legacy dashboard artifact cleanup recognizes the current V2 artifact contract before deleting an output directory, including raw-provider reference, parser, OCR, chunk, embedding, model-usage, and graph metadata artifacts. Unrecognized source-looking directories are preserved.
...
- A local smoke benchmark on 2026-05-28 generated `.local/provider-benchmark-canonical-samples.json` from the QA sample root and ran `current,docling` over two small `image_scan,newspaper_packet` samples with a 10 MB cap. After single-page packet images were excluded from page-range segmentation requirements, both providers were candidates on this narrow non-splitting slice. Current averaged about 2.92 seconds; Docling averaged about 24.91 seconds on the VM CPU path. This is not enough evidence to promote Docling as the default because it did not exercise long multi-page scrapbook/newspaper PDFs.
...
- Docling extraction itself also fails closed before converter construction when local model cache is required but missing. This protects direct provider configuration paths, not only provider-selection health checks.
...
- verified-content search via the dashboard `Search` page.
- citation-first results with file path, chunk, page range, snippet, score, and retrieval explanation.
- filter by run key, primary tag, content class, and arbitrary Qdrant metadata filters.
