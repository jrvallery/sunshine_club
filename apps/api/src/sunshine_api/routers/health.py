...
@router.get("/admin/system/postgres-runtime/runs/{run_key}/report")
def postgres_runtime_run_report(run_key: str, limit: int = 500) -> dict[str, Any]:
    try:
        report = get_postgres_run_report(run_key=run_key, limit=limit)
    except KeyError as error:
        raise HTTPException(status_code=404, detail=str(error)) from error
    except ValueError as error:
        raise HTTPException(status_code=400, detail=str(error)) from error
...
def postgres_runtime_run_events(run_key: str, limit: int = 200) -> dict[str, Any]:
    try:
        events = list_postgres_run_events(run_key=run_key, limit=limit)
    except KeyError as error:
        raise HTTPException(status_code=404, detail=str(error)) from error
    except ValueError as error:
        raise HTTPException(status_code=400, detail=str(error)) from error
    return {"ok": True, "run_key": run_key, "count": len(events), "events": events}
