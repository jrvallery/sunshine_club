...
@workflow.defn
class BatchPipelineWorkflow:
    """Durable wrapper around the LangGraph batch pipeline."""
    ...
    async def run(self, payload: dict[str, Any]) -> dict[str, Any]:
        return await workflow.execute_activity(
            ...
            payload,
            start_to_close_timeout=timedelta(hours=float(payload.get("timeout_hours") or 12)),
            retry_policy=RetryPolicy(maximum_attempts=int(payload.get("maximum_attempts") or 1)),
        )


__all__ = ["BatchPipelineWorkflow", "SingleFilePipelineWorkflow"]
